...
GO <- fread("GO_associations_MF.tsv")
...
pvalues_MF <- data.frame(GO_term = usedGO(GOdata))
for(i in 1:length(allGenes_list)){
  ...
  pvalues_MF <- merge(pvalues_MF, current_pvalue, by.x="GO_term", by.y="row.names")
  print(i)
}
...
# Calculations and plotting for MF
MF[, GO_term := NULL]                      # Remove GO_term column to leave only p-values per neighbourhood
MF_0.01 <- apply(MF, 2, below_0.01)        # Get GO terms enriched with p < 0.01
MF_0.001 <- apply(MF, 2, below_0.001)      # Get GO terms enriched with p < 0.001
MF_0.0001 <- apply(MF, 2, below_0.0001)    # Get GO terms enriched with p < 0.0001

MF <- melt( data.frame( p0.01 = MF_0.01, p0.001 = MF_0.001, p0.0001 = MF_0.0001), measure.vars = c("p0.01", "p0.001", "p0.0001"))

MF <- ggplot(MF, aes(x=variable, fill=value))+
...
ggsave("GO_MF_pies.pdf", MF, width = 10, height = 5, units = c("cm"))
