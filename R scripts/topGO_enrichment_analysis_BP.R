...
# Clear workspace
rm( list = ls()[! ls() %in% c("CP", "ProHD_proteins")] )
gc()
...
GO <- fread("GO_associations_BP.tsv")
...
for(i in 1:length(allGenes_list)){
  ...
  pvalues_BP <- merge(pvalues_BP, current_pvalue, by.x="GO_term", by.y="row.names")
  print(i)
}
...
# Calculations and plotting for BP
BP[, GO_term := NULL]                      # Remove GO_term column to leave only p-values per neighbourhood
BP_0.01 <- apply(BP, 2, below_0.01)        # Get GO terms enriched with p < 0.01
BP_0.001 <- apply(BP, 2, below_0.001)      # Get GO terms enriched with p < 0.001
BP_0.0001 <- apply(BP, 2, below_0.0001)    # Get GO terms enriched with p < 0.0001

BP <- melt( data.frame( p0.01 = BP_0.01, p0.001 = BP_0.001, p0.0001 = BP_0.0001), 
            measure.vars = c("p0.01", "p0.001", "p0.0001"))

BP <- ggplot(BP, aes(x=variable, fill=value))+
...
ggsave("GO_BP_pie.pdf", BP, width = 10, height = 5, units = c("cm"))
